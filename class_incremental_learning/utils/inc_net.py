...
class LoRAMlp(Mlp):
    ...
    def add_task(self):
        ...
        if self._cur_task == 1:
            nn.init.kaiming_uniform_(lora_A1.weight, a=math.sqrt(5))
            nn.init.zeros_(lora_B1.weight)
        else:
...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        # Replace attention modules with LoRA-enabled versions
        ...
        # Replace MLP modules with LoRA-enabled versions
        for block in self.backbone.blocks:
            mlp = block.mlp
