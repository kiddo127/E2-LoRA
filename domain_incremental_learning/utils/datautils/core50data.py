...
class CORE50(object):
    ...
    Args:
        ...
        scenario (string, optional): One of the three scenarios of the CORe50
            benchmark ``ni``, ``nc``, ``nic``, `nicv2_79`,``nicv2_196`` and
             ``nicv2_391``.
             => ``ni``: new instances, ``nc``: new classes, ``nic``: new instances;
    ...
    def __init__(self, root='', preload=True, scenario='ni', cumul=False,
                 run=0, start_batch=0, train_seq: list = None):
        """" Initialize Object """
    ...
    def __next__(self):
        ...
        # check if current session data is available
        if self.batch == self.nbatch[scen]:
            raise StopIteration

        session_id = self.train_seq[batch]

        assert session_id in list(range(9)), 'session_id should be in [0, 7]'
        ...
        if self.cumul:
            ...
            train_idx_list = self.LUP[scen][run][session_id]
        ...
        if self.cumul:
            ...
            train_y = self.labels[scen][run][session_id]
    ...
    def get_test_set(self) -> (np.ndarray, np.ndarray):
    ...
    def get_batch_from_paths(paths, compress=False, snap_dir='',
                             on_the_fly=True, verbose=False) -> np.ndarray:
